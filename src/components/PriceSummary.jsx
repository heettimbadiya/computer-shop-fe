    ...
    <div className="card bg-gradient-to-br from-primary-50 via-accent-50/50 to-primary-50 border-2 border-primary-200/50 shadow-lg">
      <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
        ...
        <div className="min-w-0">
          <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{t('customer.configurator.estimatedTotal')}</h3>
          <p className="text-xs sm:text-sm text-gray-600">{t('customer.configurator.allComponentsSelected')}</p>
        </div>
      </div>
      ...
      {selectedPartsList.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 font-medium mb-2">{t('customer.configurator.noPartSelected', { category: 'parts' })}</p>
          <p className="text-sm text-gray-500">{t('customer.configurator.selectCategory', { category: 'parts' })}</p>
        </div>
      ) : (
        <>
          ...
          {allSelected && (
            <div className="mb-3 sm:mb-4 p-2.5 sm:p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-2 animate-fade-in">
              ...
              <p className="text-xs sm:text-sm font-semibold text-emerald-800">{t('customer.configurator.allComponentsSelected')}</p>
            </div>
          )}
          ...
          <div className="border-t-2 border-primary-200 pt-4 sm:pt-6 mt-4 sm:mt-6 bg-gradient-to-r from-white to-primary-50/50 -mx-4 sm:-mx-6 px-4 sm:px-6 pb-2 rounded-b-2xl">
            <div className="flex items-center justify-between gap-3 mb-2 sm:mb-3">
              <div className="min-w-0">
                <span className="text-base sm:text-lg font-bold text-gray-900 block">{t('customer.configurator.estimatedTotal')}</span>
                <span className="text-xs text-gray-500">{t('customer.configurator.includingAllComponents')}</span>
              </div>
            ...
            <div className="flex items-start gap-2 mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-primary-200">
              ...
              <p className="text-xs text-gray-600 leading-relaxed">
                {t('customer.configurator.priceEstimate')}
              </p>
            </div>
          </div>
        </>
